<?php
/*
   Class: WalkerEdgeClassFramework
...
class WalkerEdgeClassFramework {
    ...
    private function __construct() {
        $this->edgtOptions = WalkerEdgeClassOptions::get_instance();
        $this->edgtMetaBoxes = WalkerEdgeClassMetaBoxes::get_instance();
    }
	...
	public function setSkin(WalkerEdgeClassSkinAbstract $skinObject) {
		$this->skin = $skinObject;
	}
}

/**
 * Class WalkerEdgeClassSkinManager
 *
    ...
    private function setSkin($skinName = 'edge') {
        if($skinName !== '') {
            if(file_exists(get_template_directory().'/framework/admin/skins/'.$skinName.'/skin.php')) {
                ...
                $skinName = ucfirst($skinName).'Skin'; // Skin text is part of the class name so can't be translatable
...
/**
 * Class WalkerEdgeClassSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class WalkerEdgeClassSkinAbstract {
    /**
     * @var string
     */
    protected $skinName;
    ...
    public function loadTemplatePart($template, $params = array()) {
        ...
        if($template !== '') {
            include(walker_edge_find_template_path('framework/admin/skins/'.$this->skinName.'/templates/'.$template.'.php'));
        }
    }
    ...
    public function setShortcodeJSParams() { ?>
		<script>
			window.edgtSCIcon = '<?php echo walker_edge_get_skin_uri().'/assets/img/admin-logo-icon.png'; ?>';
...
class WalkerEdgeClassOptions {
    ...
    public function getOptionValue($key) {
        ...
        if(array_key_exists($key, $walker_edge_options)) {
            return $walker_edge_options[$key];
...
/*
   Class: WalkerEdgeClassAdminPage
   A class that initializes Edge Admin Page
*/
class WalkerEdgeClassAdminPage implements iWalkerEdgeInterfaceLayoutNode {
    ...
    function __construct($slug="", $title_label="", $icon = "") {
        $this->layout = array();
        $this->factory = new WalkerEdgeClassFieldFactory();
        $this->slug = $slug;
    ...
    public function renderChild(iWalkerEdgeInterfaceRender $child) {
        $child->render($this->factory);
    }
}

/*
   Class: WalkerEdgeClassMetaBoxes
...
/*
   Class: WalkerEdgeClassMetaBox
   A class that initializes Edge Meta Box
*/
class WalkerEdgeClassMetaBox implements iWalkerEdgeInterfaceLayoutNode {
	...
	public function setTitle($label) {
		...
		return $walker_edge_Framework->getSkin()->getSkinLabel().' '.$label;
 	}
}

if ( ! function_exists( 'walker_edge_init_framework_variable' ) ) {

    function walker_edge_init_framework_variable() {

        global $walker_edge_Framework;

        $walker_edge_Framework = WalkerEdgeClassFramework::get_instance();
        $edgtSkinManager = new WalkerEdgeClassSkinManager();
        $walker_edge_Framework->setSkin($edgtSkinManager->getSkin());
    }

    add_action('walker_edge_before_options_map', 'walker_edge_init_framework_variable' );
}
?>