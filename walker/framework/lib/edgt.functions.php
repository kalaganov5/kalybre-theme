 ...
 * It first checks global $walker_edge_options variable and if option does'nt exists there
 ...
 * @return bool
 */
function walker_edge_option_has_value($name) {
	global $walker_edge_options;
	global $walker_edge_Framework;

	if(array_key_exists($name, $walker_edge_Framework->edgtOptions->options)) {
		if(isset($walker_edge_options[$name])) {
			return true;
		} else {
			return false;
		}
 ...
 * It first checks if option exists in $walker_edge_options global array and if it does'nt exists there
 ...
 * @return mixed|void
 */
function walker_edge_option_get_value($name) {
	global $walker_edge_options;
	global $walker_edge_Framework;

	if(array_key_exists($name, $walker_edge_Framework->edgtOptions->options)) {
		if(isset($walker_edge_options[$name])) {
			return $walker_edge_options[$name];
		} else {
			return $walker_edge_Framework->edgtOptions->getOption($name);
		}
	} else {
		global $post;
		...
		if(isset($value) && $value !== "") {
			...
			return $walker_edge_Framework->edgtMetaBoxes->getOption($name);
		}
 ...
 * @return bool|string
 *
 * @see walker_edge_get_attachment_id_from_url()
 */
function walker_edge_get_attachment_thumb_url($attachment_url) {
	$attachment_id = walker_edge_get_attachment_id_from_url($attachment_url);
 ...
 */
function walker_edge_enqueue_skin_style($handle, $src, $deps = array(), $ver = false, $media = 'all') {
	global $walker_edge_Framework;

	$src = get_template_directory_uri().'/framework/admin/skins/'.$walker_edge_Framework->getSkin().'/'.$src;
 ...
 */
function walker_edge_register_skin_style($handle, $src, $deps = array(), $ver = false, $media = 'all') {
	global $walker_edge_Framework;
 ...
 */
function walker_edge_enqueue_skin_script($handle, $src, $deps = array(), $ver = false, $in_footer = false) {
	global $walker_edge_Framework;
 ...
 */
function walker_edge_register_skin_script($handle, $src, $deps = array(), $ver = false, $in_footer = false) {
	global $walker_edge_Framework;
...
if(!function_exists('walker_edge_generate_dynamic_css_and_js')) {
	/**
	 * Function that gets content of dynamic assets files and puts that in static ones
	 */
	function walker_edge_generate_dynamic_css_and_js() {
		global $wp_filesystem;
		WP_Filesystem();
		
		if ( walker_edge_is_css_folder_writable() ) {
			...
			$css = ob_get_clean();
			if ( is_multisite() ) {
				$wp_filesystem->put_contents( $css_dir . 'style_dynamic_ms_id_' . walker_edge_get_multisite_blog_id() . '.css', $css );
			} else {
			...
			$css = ob_get_clean();
			if ( is_multisite() ) {
				$wp_filesystem->put_contents( $css_dir . 'style_dynamic_responsive_ms_id_' . walker_edge_get_multisite_blog_id() . '.css', $css );
			} else {
	...
	add_action('walker_edge_after_theme_option_save', 'walker_edge_generate_dynamic_css_and_js');
}

if(!function_exists('walker_edge_gallery_upload_get_images')) {
	/**
	 * Function that outputs single image html that is used in multiple image upload field
	 */
	function walker_edge_gallery_upload_get_images() {
	...
	add_action('wp_ajax_walker_edge_gallery_upload_get_images', 'walker_edge_gallery_upload_get_images');
}

if(!function_exists('walker_edge_hex2rgb')) {
	/**
	 ...
	 */
	function walker_edge_hex2rgb($hex) {
		$hex = str_replace("#", "", $hex);
...
if(!function_exists('walker_edge_addslashes')) {
	/**
	 ...
	 */
	function walker_edge_addslashes($str) {
...
if(!function_exists('walker_edge_get_attachment_meta')) {
	/**
	 ...
	 * @return array|mixed
	 */
	function walker_edge_get_attachment_meta($attachment_id, $keys = array()) {
		$meta_data = array();
...
if(!function_exists('walker_edge_get_attachment_id_from_url')) {
	/**
	 ...
	 * @return null|string
	 */
	function walker_edge_get_attachment_id_from_url($attachment_url) {
		global $wpdb;
		$attachment_id = '';
...
if(!function_exists('walker_edge_get_attachment_meta_from_url')) {
	/**
	 ...
	 *
	 * @see walker_edge_get_attachment_id_from_url()
	 * @see walker_edge_get_attachment_meta()
	 *
	 * @version 0.1
	 */
	function walker_edge_get_attachment_meta_from_url($attachment_url, $keys = array()) {
		...
		if(!empty($attachment_id)) {
			//get post meta
			$attachment_meta = walker_edge_get_attachment_meta($attachment_id, $keys);
		}
...
if(!function_exists('walker_edge_get_image_dimensions')) {
	/**
	 ...
	 *
	 * @see  walker_edge_get_attachment_meta_from_url()
	 * @uses getimagesize
	 *
	 * @version 0.1
	 */
	function walker_edge_get_image_dimensions($url) {
		$image_sizes = array();

		//is url passed?
		if($url !== '') {
			//get image sizes from posts meta if attachment exists
			$image_sizes = walker_edge_get_attachment_meta_from_url($url, array('width', 'height'));
...
if(!function_exists('walker_edge_get_native_fonts_list')) {
	/**
	 * Function that returns array of native fonts
	 * @return array
	 */
	function walker_edge_get_native_fonts_list() {
...
if(!function_exists('walker_edge_get_native_fonts_array')) {
	/**
	 * Function that returns formatted array of native fonts
	 *
	 * @uses walker_edge_get_native_fonts_list()
	 * @return array
	 */
	function walker_edge_get_native_fonts_array() {
		$native_fonts_list  = walker_edge_get_native_fonts_list();
		$native_font_index  = 0;
...
if(!function_exists('walker_edge_is_native_font')) {
	/**
	 ...
	 * @return bool
	 */
	function walker_edge_is_native_font($font_family) {
		return in_array(str_replace('+', ' ', $font_family), walker_edge_get_native_fonts_list());
	}
}


if(!function_exists('walker_edge_merge_fonts')) {
	/**
	 * Function that merge google and native fonts
	 *
	 * @uses walker_edge_get_native_fonts_array()
	 * @return array
	 */
	function walker_edge_merge_fonts() {
		global $walker_edge_fonts_array;
		$native_fonts = walker_edge_get_native_fonts_array();

		if(is_array($native_fonts) && count($native_fonts)) {
			$walker_edge_fonts_array = array_merge($native_fonts, $walker_edge_fonts_array);
		}
	}

	add_action('admin_init', 'walker_edge_merge_fonts');
}

if(!function_exists('walker_edge_is_css_folder_writable')) {
	/**
	 ...
	 * @uses is_writable()
	 */
	function walker_edge_is_css_folder_writable() {
		$css_dir = EDGE_ASSETS_ROOT_DIR.'/css';
...
if(!function_exists('walker_edge_is_js_folder_writable')) {
	/**
	 ...
	 * @uses is_writable()
	 */
	function walker_edge_is_js_folder_writable() {
...
if(!function_exists('walker_edge_assets_folders_writable')) {
	/**
	 * Function that if css and js folders are writable
	 * @return bool
	 *
	 * @version 0.1
	 * @see walker_edge_is_css_folder_writable()
	 * @see walker_edge_is_js_folder_writable()
	 */
	function walker_edge_assets_folders_writable() {
		return walker_edge_is_css_folder_writable() && walker_edge_is_js_folder_writable();
	}
}

if ( ! function_exists( 'walker_edge_get_multisite_blog_id' ) ) {
	/**
	 * Check is multisite and return blog id
	 *
	 * @return int
	 */
	function walker_edge_get_multisite_blog_id() {
		if ( is_multisite() ) {
...
if(!function_exists('walker_edge_writable_assets_folders_notice')) {
	 ...
	 */
	function walker_edge_writable_assets_folders_notice() {
		global $pagenow;

		$is_theme_options_page = isset($_GET['page']) && strstr($_GET['page'], 'walker_edge_theme_menu');

		if($pagenow === 'admin.php' && $is_theme_options_page) {
			if(!walker_edge_assets_folders_writable()) { ?>
				<div class="error">
					<p><?php esc_html_e('Note that writing permissions aren\'t set for folders containing css and js files on your server. We recommend setting writing permissions in order to optimize your site performance. For further instructions, please refer to our documentation.', 'walker'); ?></p>
				</div>
			<?php }
		}
	}

	add_action('admin_notices', 'walker_edge_writable_assets_folders_notice');
}
...
if(!function_exists('walker_edge_inline_attr')) {
	/**
	 * Function that generates html attribute
	 *
	 * @param $value string | array value of html attribute
	 ...
	 * @return string generated html attribute
	 */
	function walker_edge_inline_attr($value, $attr, $glue = '') {
		echo walker_edge_get_inline_attr($value, $attr, $glue);
	}
}

if(!function_exists('walker_edge_get_inline_attrs')) {
	/**
	 ...
	 * @return string
	 */
	function walker_edge_get_inline_attrs($attrs) {
		$output = '';

		if(is_array($attrs) && count($attrs)) {
			foreach($attrs as $attr => $value) {
				$output .= ' '.walker_edge_get_inline_attr($value, $attr);
			}
		}
...
if(!function_exists('walker_edge_get_skin_uri')) {
	/**
	 * Returns current skin URI
	 * @return mixed
	 */
	function walker_edge_get_skin_uri() {
		global $walker_edge_Framework;

		$current_skin = $walker_edge_Framework->getSkin();

		return $current_skin->getSkinURI();
	}
}

if(!function_exists('walker_edge_core_installed')) {
	/**
	 * Checks if core plugin is installed
	 * @return bool
	 */
	function walker_edge_core_installed() {
		return defined('EDGE_CORE_VERSION');
	}
}

if(!function_exists('walker_edge_resize_image')) {
	/**
	 ...
	 */
	function walker_edge_resize_image($attach_id = null, $attach_url = null, $width = null, $height = null, $crop = true) {
		$return_array = array();

		//is attachment id empty?
		if(empty($attach_id) && $attach_url !== '') {
			//get attachment id from url
			$attach_id = walker_edge_get_attachment_id_from_url($attach_url);
		}
...
if(!function_exists('walker_edge_generate_thumbnail')) {
	/**
	 * Generates thumbnail img tag. It calls walker_edge_resize_image function which resizes img on the fly
	 *
	 ...
	 *
	 * @see walker_edge_resize_image()
	 * @see walker_edge_get_attachment_id_from_url()
	 */
	function walker_edge_generate_thumbnail($attach_id = null, $attach_url = null, $width = null, $height = null, $crop = true) {
		//is attachment id empty?
		if(empty($attach_id)) {
		...
		if(!empty($attach_id) || !empty($attach_url)) {
			$img_info = walker_edge_resize_image($attach_id, $attach_url, $width, $height, $crop);
			$img_alt  = !empty($attach_id) ? get_post_meta($attach_id, '_wp_attachment_image_alt', true) : '';

			if(is_array($img_info) && count($img_info)) {
				return '<img src="'.$img_info['img_url'].'" alt="'.esc_attr($img_alt).'" width="'.$img_info['img_width'].'" height="'.$img_info['img_height'].'" />';
			}
		}
...
if(!function_exists('walker_edge_get_template_part')) {
	/**
	 ...
	 */
	function walker_edge_get_template_part($template, $slug = '', $params = array()) {
		...
		$located = walker_edge_find_template_path($templates);
...
if(!function_exists('walker_edge_get_module_template_part')) {
	/**
	 ...
	 */
	function walker_edge_get_module_template_part($template, $module, $slug = '', $params = array()) {
		$template_path = 'framework/modules/'.$module;

		walker_edge_get_template_part($template_path.'/'.$template, $slug, $params);
	}
}


if(!function_exists('walker_edge_get_shortcode_module_template_part')) {
	/**
	 * Loads module template part.
	 *
	 * @param string $template name of the template to load
	 ...
	 * @see walker_edge_get_template_part()
	 */
	function walker_edge_get_shortcode_module_template_part($template, $module, $slug = '', $params = array()) {

		//HTML Content from template
		$html          = '';
		$template_path = EDGE_CORE_ABS_PATH . '/shortcodes/'.$module;
...
if(!function_exists('walker_edge_get_blog_module_template_part')) {
	/**
	 * Loads module template part.
	 *
	 * @param string $template name of the template to load
	 ...
	 * @see walker_edge_get_template_part()
	 */
	function walker_edge_get_blog_module_template_part($module, $slug = '', $params = array()) {
...
if(!function_exists('walker_edge_find_template_path')) {
	/**
	 ...
	 * @return string
	 */
	function walker_edge_find_template_path($template_names) {
		$located = '';
...
if(!function_exists('walker_edge_filter_suffix')) {
	/**
	 ...
	 * @return string
	 */
	function walker_edge_filter_suffix($value, $suffix) {
		if($value !== '' && walker_edge_string_ends_with($value, $suffix)) {
...
if(!function_exists('walker_edge_filter_px')) {
	/**
	 ...
	 * @return string
	 *
	 * @see walker_edge_filter_suffix
	 */
	function walker_edge_filter_px($value) {
		return walker_edge_filter_suffix($value, 'px');
	}
}

if(!function_exists('walker_edge_string_starts_with')) {
	/**
	 ...
	 * @return bool
	 */
	function walker_edge_string_starts_with($haystack, $needle) {
		if($haystack !== '' && $needle !== '') {
...
if(!function_exists('walker_edge_string_ends_with')) {
	/**
	 ...
	 * @return bool
	 */
	function walker_edge_string_ends_with($haystack, $needle) {
		if($haystack !== '' && $needle !== '') {
...
if(!function_exists('walker_edge_dynamic_css')) {
	/**
	 ...
	 *      header { width: 100%; }
	 * function call should look like this: walker_edge_dynamic_css('header', array('width' => '100%'));
	 *
	 * @return string
	 */
	function walker_edge_dynamic_css($selector, $rules) {
		$output = '';
...
if(!function_exists('walker_edge_get_formatted_font_family')) {
	/**
	 ...
	 * @return mixed
	 */
	function walker_edge_get_formatted_font_family($value) {
		return str_replace('+', ' ', $value);
	}
}

if(!function_exists('walker_edge_active_widget')) {
	/**
	 * Whether widget is displayed on the front-end.
	 */
	function walker_edge_active_widget($callback = false, $widget_id = false, $id_base = false, $skip_inactive = true) {
...
if(!function_exists('walker_edge_execute_shortcode')) {
	/**
	 ...
	 * @return mixed|string
	 */
	function walker_edge_execute_shortcode($shortcode_tag, $atts, $content = null) {
...
if(!function_exists('walker_edge_paged')) {
	/**
	 * Function which return number of pages depends on query
	 *
	 * @return int
	 */
	function walker_edge_paged() {
...
if(!function_exists('walker_edge_is_smoth_scroll_enabled')) {
	/**
	 * Function that check is smoth scroll is enabled
	 */
	function walker_edge_is_smoth_scroll_enabled() {
		$is_enabled = false;

		//is smooth scroll enabled enabled and not Mac device?
		if(walker_edge_options()->getOptionValue('smooth_scroll') == 'yes' && strpos( getenv( 'HTTP_USER_AGENT' ), "Macintosh; Intel Mac OS X" ) === false ) {
			$is_enabled = true;
		}
...
if(!function_exists('walker_edge_show_comments')) {
	/**
	 * Functions which check are comments enabled on page
	 *
	 * @return boolean
	 */
	function walker_edge_show_comments() {
		...
		if(get_post_meta($id, 'edgtf_page_comments_meta', true) == 'yes') {
			...
			if(is_page() && walker_edge_options()->getOptionValue('page_show_comments') == 'yes') {
				$comments = true;
			} elseif(is_singular('post') && walker_edge_options()->getOptionValue('blog_single_comments') == 'yes') {
				$comments = true;
			} elseif(is_singular('portfolio-item') && walker_edge_options()->getOptionValue('portfolio_single_comments') == 'yes') {
				$comments = true;
			}
		}
...
if(!function_exists('walker_edge_page_comments')) {
	/**
	 * Functions which show comments on page
	 *
	 * @return boolean
	 */
	function walker_edge_page_comments() {

		$comments = walker_edge_show_comments();
	...
	add_action('walker_edge_page_after_content', 'walker_edge_page_comments');
}

if(!function_exists('walker_edge_page_show_pagination')) {
	/**
	 * Functions which show pagination on pages
	 *
	 * @return boolean
	 */
	function walker_edge_page_show_pagination() {
...
if(!function_exists('walker_edge_icon_collections')) {
	/**
	 * Returns instance of WalkerEdgeClassIconCollections class
	 *
	 * @return WalkerEdgeClassIconCollections
	 */
	function walker_edge_icon_collections() {
		return WalkerEdgeClassIconCollections::get_instance();
	}
}

if(!function_exists('walker_edge_get_meta_field_intersect')) {
	/**
	 * @param $name
	 * @param $post_id
	 *
	 ...
	 */
	function walker_edge_get_meta_field_intersect($name, $post_id = '') {
		$post_id = !empty($post_id) ? $post_id : get_the_ID();

		$value = walker_edge_options()->getOptionValue($name);
		...
		$value = apply_filters('walker_edge_meta_field_intersect_'.$name, $value);

		return $value;
	}
}

if (!function_exists('walker_edge_paged_home')) {
	/**
	 * Set false to on paged url on homepage
	 *
	 * @return int
	 */
	function walker_edge_paged_home() {
		global $wp_query;
	...
	add_filter('redirect_canonical', 'walker_edge_paged_home');
}

if(!function_exists('walker_edge_register_button')) {
	/**
	 ...
	 * @return mixed
	 */
	function walker_edge_register_button($buttons) {
...
if(!function_exists('walker_edge_add_plugin')) {

	function walker_edge_add_plugin($plugin_array) {
...
if(!function_exists('walker_edge_shortcodes_button')) {
	/**
	 * Register Edge Shortcodes in WP Editor
	 */
	function walker_edge_shortcodes_button() {
		if(!current_user_can('edit_posts') && !current_user_can('edit_pages')) {
			return;
		}

		if(get_user_option('rich_editing') == 'true') {
			add_filter('mce_external_plugins', 'walker_edge_add_plugin');
			add_filter('mce_buttons', 'walker_edge_register_button');
		}
	}
}

add_action('after_setup_theme', 'walker_edge_shortcodes_button');