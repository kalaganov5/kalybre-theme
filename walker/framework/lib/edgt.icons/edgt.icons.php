...
/*
  Class: WalkerEdgeClassIconCollections
...
class WalkerEdgeClassIconCollections {
    ...
    private function initIconCollections() {
        $this->addIconCollection('font_awesome', new WalkerEdgeClassIconsFontAwesome("Font Awesome", "fa_icon"));
        $this->addIconCollection('font_elegant', new WalkerEdgeClassIconsFontElegant("Font Elegant", "fe_icon"));
        $this->addIconCollection('ion_icons', new WalkerEdgeClassIonIcons("Ion Icons", "ion_icon"));
        $this->addIconCollection('linea_icons', new WalkerEdgeClassLineaIcons('Linea Icons', 'linea_icon'));
    }
    ...
    public function getVCParamsArray($iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false) {
        ...
        $iconPackParams = array(
            'type'        => 'dropdown',
            'heading'     =>  esc_html__( 'Icon Pack', 'walker' ),
        ...
        if(is_array($this->iconCollections) && count($this->iconCollections)) {
            foreach($this->iconCollections as $key => $collection) {
                $iconSetParams[] = array(
                    'type'        => 'dropdown',
                    'heading'     =>  esc_html__( 'Icon', 'walker' ),
    ...
    public function getSocialIconWidgetParamsArray() {
        ...
        $iconPackParams[] = array(
            'type'    => 'dropdown',
            'title' => esc_html__( 'Icon Pack', 'walker' ),
            'name'    => 'icon_pack',
            'options' => array(
                'font_awesome' => esc_html__( 'Font Awesome', 'walker' ),
                'font_elegant' => esc_html__( 'Font Elegant', 'walker' ),
                'ion_icons' => esc_html__( 'Ion Icons', 'walker' )
            )
        );
    ...
    public function getIconSizesArray() {
        return array(
            esc_html__( "Tiny", 'walker' ) => "fa-lg",
            esc_html__( "Small", 'walker' ) => "fa-2x",
            esc_html__( "Medium", 'walker' ) => "fa-3x",
            esc_html__( "Large", 'walker' ) => "fa-4x",
            esc_html__( "Very Large", 'walker' ) => "fa-5x"
        );
    }
    ...
    public function getIconCollectionIcons(iWalkerEdgeInterfaceIconCollection $collection) {
    ...
    public function enqueueStyles() {
        if(is_array($this->iconCollections) && count($this->iconCollections)) {
            foreach($this->iconCollections as $collection_key => $collection_obj) {
                wp_enqueue_style('walker-'.$collection_key, $collection_obj->styleUrl);
            }
        }
    }
...
if ( ! function_exists( 'walker_edge_activate_theme_icons' ) ) {

    function walker_edge_activate_theme_icons() {
        global $walker_edge_IconCollections;
        $walker_edge_IconCollections = WalkerEdgeClassIconCollections::get_instance();
    }

    add_action('after_setup_theme', 'walker_edge_activate_theme_icons');
}