<?php
class WalkerEdgeClassLike {

	private static $instance;

	private function __construct() {
		add_action('wp_enqueue_scripts', array( $this, 'enqueue_scripts'));
		add_action('wp_ajax_walker_edge_like', array( $this, 'ajax'));
		add_action('wp_ajax_nopriv_walker_edge_like', array( $this, 'ajax'));
	}
	...
	function enqueue_scripts() {
		...
		wp_enqueue_script( 'walker-like', EDGE_ASSETS_ROOT . '/js/like.js', 'jquery', '1.0', TRUE );

		wp_localize_script( 'walker-like', 'edgtfLike', array(
	...
	public function add_like() {
		...
		$title = esc_html__('Like this', 'walker');
		if( isset($_COOKIE['edgtf-like_'. $post->ID]) ){
			$class = 'edgtf-like liked';
			$title = esc_html__('You already like this!', 'walker');
		}
...
if ( !function_exists( 'walker_edge_create_like' ) ) {

    function walker_edge_create_like() {
        WalkerEdgeClassLike::get_instance();
    }

    add_action('after_setup_theme', 'walker_edge_create_like');
}