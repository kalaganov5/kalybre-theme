...
if(!function_exists('walker_edge_single_portfolio')) {
    function walker_edge_single_portfolio() {
        $portfolio_template = walker_edge_get_portfolio_single_type();
        ...
        if ($portfolio_template == 'gallery') {
            $params['holder_class'][] = 'edgtf-portfolio-gallery-' . walker_edge_options()->getOptionValue('portfolio_single_numb_columns');
        }

        walker_edge_get_module_template_part('templates/single/holder', 'portfolio', '', $params);
    }
}

if(!function_exists('walker_edge_get_portfolio_single_type')) {
    function walker_edge_get_portfolio_single_type() {
        return walker_edge_get_meta_field_intersect('portfolio_single_template');
    }
}

if(!function_exists('walker_edge_get_portfolio_single_media')) {
    function walker_edge_get_portfolio_single_media() {
        ...
        if(is_array($videos) && count($videos)) {
            usort($videos, 'walker_edge_compare_portfolio_videos');
            foreach($videos as $video) {
                ...
                if(!empty($video['portfoliovideotype'])) {
                    ...
                    $media['video_url']   = walker_edge_portfolio_get_video_url($video);
...
if(!function_exists('walker_edge_portfolio_get_video_url')) {
    function walker_edge_portfolio_get_video_url($video) {
...
if(!function_exists('walker_edge_portfolio_get_media_html')) {
    function walker_edge_portfolio_get_media_html($media) {
        global $wp_filesystem;
        $params = array();

        //For adding image overlay in gallery template type
        $params['gallery'] = (walker_edge_get_portfolio_single_type() == 'gallery') ? true : false;

        if($media['type'] == 'image') {
            $params['lightbox'] = walker_edge_options()->getOptionValue('portfolio_single_lightbox_images') == 'yes';
        ...
        if(in_array($media['type'], array('youtube', 'vimeo'))) {
            $params['lightbox'] = walker_edge_options()->getOptionValue('portfolio_single_lightbox_videos') == 'yes';
        ...
        walker_edge_get_module_template_part('templates/single/media/'.$media['type'], 'portfolio', '', $params);
    }
}

if(!function_exists('walker_edge_compare_portfolio_videos')) {
    /**
     ...
     */
    function walker_edge_compare_portfolio_videos($a, $b) {
...
if(!function_exists('walker_edge_compare_portfolio_options')) {
    /**
     ...
     * @return int result of comparison
     */
    function walker_edge_compare_portfolio_options($a, $b) {
...
if(!function_exists('walker_edge_portfolio_get_info_part')) {
    function walker_edge_portfolio_get_info_part($part) {
        $portfolio_template = walker_edge_get_portfolio_single_type();

        walker_edge_get_module_template_part('templates/single/parts/'.$part, 'portfolio', $portfolio_template);
    }
}