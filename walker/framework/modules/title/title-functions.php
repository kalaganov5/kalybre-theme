...
if(!function_exists('walker_edge_get_title')) {
    /**
     * Loads title area HTML
     */
    function walker_edge_get_title() {
        $id = walker_edge_get_page_id();

        extract(walker_edge_title_area_height());
        extract(walker_edge_title_area_background());
        ...
        if(($meta_temp = get_post_meta($id, "edgtf_show_title_area_meta", true)) !== ""){
            ...
            $show_title_area = walker_edge_options()->getOptionValue('show_title_area') == 'yes' ? true : false;
        }

        if(walker_edge_is_woocommerce_installed() && is_singular('product')) {
            $woo_temp = get_post_meta(get_the_ID(), "edgtf_woo_show_title_area_meta", true);
            if($woo_temp === "no"){
        ...
        if(($meta_temp = get_post_meta($id, "edgtf_title_area_type_meta", true)) !== ""){
            ...
            $type = walker_edge_options()->getOptionValue('title_area_type');
        }
        ...
        if(($meta_temp = get_post_meta($id, "edgtf_title_area_enable_breadcrumbs_meta", true)) !== ""){
            ...
            $enable_breadcrumbs = walker_edge_options()->getOptionValue('title_area_enable_breadcrumbs') == 'yes' ? true : false;
        }
        ...
        $parameters = apply_filters('walker_edge_title_area_height_params', $parameters);

        walker_edge_get_module_template_part('templates/title', 'title', '', $parameters);
    }
}

if(!function_exists('walker_edge_get_title_text')) {
    /**
     * Function that returns current page title text. Defines walker_edge_title_text filter
     ...
     */
    function walker_edge_get_title_text() {
        ...
        if (is_tag()) {
            ...
            $title = single_term_title("", false).esc_html__(' Tag', 'walker');
        }
        ...
        elseif (is_author()) {
            ...
            $title = esc_html__('Author:', 'walker') . " " . get_the_author();
        }
        ...
        elseif (is_search()) {
            ...
            $title = esc_html__('Search results for: ', 'walker') . get_search_query();
        }

        //is single post page
        elseif (is_singular('post')) {
            if (walker_edge_options()->getOptionValue('blog_single_title_in_title_area') == "yes") {
        ...
        elseif (is_404()) {
            ...
            if(walker_edge_options()->getOptionValue('404_title') != "") {
                //get it from options
                $title = walker_edge_options()->getOptionValue('404_title');
            } else {
                //get default 404 page title
                $title = esc_html__('404 - Page not found', 'walker');
            }
        }

        //is WooCommerce installed and is shop or single product page?
        elseif(walker_edge_is_woocommerce_installed() && (walker_edge_is_woocommerce_shop() || is_singular('product'))) {
        ...
        elseif(walker_edge_is_woocommerce_installed() && (is_product_category() || is_product_tag())) {
            global $wp_query;
        ...
        elseif (is_archive()) {
            $title = esc_html__('Archive','walker');
        }
        ...
        $title = apply_filters('walker_edge_title_text', $title);

        return $title;
    }
}

if(!function_exists('walker_edge_title_text')) {
    /**
     * Function that echoes title text.
     *
     * @see walker_edge_get_title_text()
     */
    function walker_edge_title_text() {
        echo walker_edge_get_title_text();
    }
}

if(!function_exists('walker_edge_subtitle_text')) {
    /**
     * Function that echoes subtitle text.
     *
     */
    function walker_edge_subtitle_text() {
        $id = walker_edge_get_page_id();
        $subtitle 	= '';
...
if(!function_exists('walker_edge_custom_breadcrumbs')) {
    /**
     ...
     * @see is_front_page()
     * @see is_category()
     * @see walker_edge_is_product_category()
     * @see get_search_query()
     */
    function walker_edge_custom_breadcrumbs() {
        global $post, $wp_query;

        $output = "";
        $homeLink = esc_url(home_url('/'));
        $pageid = walker_edge_get_page_id();
        $bread_style = "";
        ...
        $delimiter = '<span class="edgtf-delimiter" '.walker_edge_get_inline_style($bread_style).'>&nbsp; / &nbsp;</span>'; // delimiter between crumbs
        $home = esc_html__('Home', 'walker'); // text for the 'Home' link
        $showCurrent = 1; // 1 - show current post/page title in breadcrumbs, 0 - don't show
        $before = '<span class="edgtf-current" '.walker_edge_get_inline_style($bread_style).'>'; // tag before the current crumb
        $after = '</span>'; // tag after the current crumb

        if (is_home() && !is_front_page()) {
            $output = '<div class="edgtf-breadcrumbs"><div class="edgtf-breadcrumbs-inner" itemprop="breadcrumb"><a itemprop="url" '.walker_edge_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a>' . $delimiter . ' <a itemprop="url" '.walker_edge_get_inline_style($bread_style).' href="' . $homeLink . '">'. get_the_title($pageid) .'</a></div></div>';
        ...
        elseif(is_front_page()) {
            if ($showOnHome == 1) $output = '<div class="edgtf-breadcrumbs"><div class="edgtf-breadcrumbs-inner" itemprop="breadcrumb"><a itemprop="url" '.walker_edge_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a></div></div>';
        }

        else {

            $output .= '<div class="edgtf-breadcrumbs"><div class="edgtf-breadcrumbs-inner" itemprop="breadcrumb"><a itemprop="url" '.walker_edge_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a>' . $delimiter;

            if ( is_category() || walker_edge_is_product_category()) {
                ...
                $output .= $before . esc_html__('Search results for ', 'walker') . '"' . get_search_query() . '"' . $after;

            } elseif ( is_day() ) {
                $output .= '<a itemprop="url" '.walker_edge_get_inline_style($bread_style).' href="' . get_year_link(get_the_time('Y')) . '">' . get_the_time('Y') . '</a>' . $delimiter;
                $output .= '<a itemprop="url" '.walker_edge_get_inline_style($bread_style).' href="' . get_month_link(get_the_time('Y'),get_the_time('m')) . '">' . get_the_time('F') . '</a>' . $delimiter;
            ...
            } elseif ( walker_edge_is_woocommerce_installed() && is_singular('product') ){
                global $product;
                if(get_option('woocommerce_shop_page_id')){
                    $output .= '<a itemprop="url" '. walker_edge_get_inline_style($bread_style) .' href="' . get_permalink(get_option('woocommerce_shop_page_id')) . '">' . get_the_title(get_option('woocommerce_shop_page_id')) . '</a>' . $delimiter;
                }
                ...
                $output .= '<a itemprop="url" '.walker_edge_get_inline_style($bread_style).' href="' . get_permalink($parent) . '">' . $parent->post_title . '</a>';
                ...
                while ($parent_id) {
                    ...
                    $breadcrumbs[] = '<a itemprop="url" '.walker_edge_get_inline_style($bread_style).' href="' . get_permalink($page->ID) . '">' . get_the_title($page->ID) . '</a>';
                ...
                $output .= $before . esc_html__('Posts tagged ','walker') .'"' . single_tag_title('', false) . '"' . $after;

            } elseif ( is_author() ) {
                global $authordata;
                $output .= $before . esc_html__('Articles posted by ','walker') . $authordata->display_name . $after;

            } elseif ( is_404() ) {
                $output .= $before . 'Error 404' . $after;
            } elseif(walker_edge_is_woocommerce_installed() && walker_edge_is_woocommerce_shop()){
                global $woocommerce;
                $shop_id = get_option('woocommerce_shop_page_id');
            ...
            if ( get_query_var('paged') ) {
                ...
                $output .= $before . " (" . esc_html__('Page', 'walker') . ' ' . get_query_var('paged') . ")" . $after;
...
if(!function_exists('walker_edge_get_title_area_height')) {

    /**
     * Function that returns title height
     **/
    function walker_edge_get_title_area_height() {
        ...
        $title_height = 76; // default title height without header height
        ...
        } elseif(walker_edge_options()->getOptionValue('title_area_height') !== '') {
            $title_height = walker_edge_options()->getOptionValue('title_area_height');
        }

        return apply_filters('walker_edge_title_area_height', $title_height);
    }
}

if(!function_exists('walker_edge_get_title_content_padding')) {
    /**
     * Function that returns title content pading
     **/

    function walker_edge_get_title_content_padding() {
		$title_padding = apply_filters('walker_edge_filter_title_content_padding', 0);
...
if(!function_exists('walker_edge_title_area_height')) {
    /**
     * Function that returns title height and padding to be applied in template
     **/

    function walker_edge_title_area_height() {
        $id = walker_edge_get_page_id();
        $title_height_and_padding = array();
        $title_height          = walker_edge_get_title_area_height();
        $header_height_padding = walker_edge_get_title_content_padding();
        ...
        if(($meta_temp = get_post_meta($id, "edgtf_title_area_vertial_alignment_meta", true)) !== '') {
            ...
            $title_vertical_alignment = walker_edge_options()->getOptionValue('title_area_vertial_alignment');;
        }
...
if(!function_exists('walker_edge_title_area_background')) {
    /**
     * Function that returns title background style be applied in template
     **/

    function walker_edge_title_area_background() {
        $id = walker_edge_get_page_id();
        $show_title_img = true;
        ...
        if(($meta_temp = get_post_meta($id, "edgtf_title_area_background_color_meta", true)) != ""){
            ...
            $background_color = walker_edge_options()->getOptionValue('title_area_background_color');
        }
        ...
        if(($meta_temp = get_post_meta($id, "edgtf_title_area_background_image_meta", true)) != ""){
            ...
            $background_image = walker_edge_options()->getOptionValue('title_area_background_image');
        }

        //check for background image width
        $background_image_width = "";
        if($background_image !== ''){
            $background_image_width_dimensions_array = walker_edge_get_image_dimensions($background_image);
