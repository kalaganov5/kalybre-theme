 ...
 */
class WalkerEdgeClassSocialIconWidget extends WalkerEdgeClassWidget {
	/**
	 * Set basic widget options and call parent class construct
	 */
	public function __construct() {
		parent::__construct(
			'edgtf_social_icon_widget', // Base ID
			esc_html__( 'Walker Social Icon Widget', 'walker' ),
			array( 'description' => esc_html__( 'Add social network icons to widget areas', 'walker' ) )
		);
	...
	protected function setParams() {
		$this->params = array_merge(
			walker_edge_icon_collections()->getSocialIconWidgetParamsArray(),
			array(
				array(
					'type'        => 'textfield',
				...
				array(
					'type'        => 'textfield',
					'title'       => esc_html__( 'Text Size (px)', 'walker' ),
					'name'        => 'text_size'
				),
				array(
					'type'        => 'textfield',
				...
				array(
					'type'        => 'textfield',
					'title'       => esc_html__( 'Hover Color', 'walker' ),
					'name'        => 'hover_color'
				),
				array(
					'type'        => 'textfield',
					'title'       => esc_html__( 'Margin', 'walker' ),
					'name'        => 'margin',
					'description' => esc_html__( 'Margin (top right bottom left)', 'walker' )
				)
			)
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		if ( ! empty( $instance['text_size'] ) && $instance['text_size'] !== '' ) {
			$social_icon_styles[] = 'font-size: ' . walker_edge_filter_px( $instance['text_size'] ) . 'px';
		}
		...
		?>
		<a class="edgtf-social-icon-widget-holder" <?php echo walker_edge_get_inline_attr( $hover_color, 'data-hover-color' ); ?> <?php echo walker_edge_get_inline_attr( $original_color, 'data-original-color' ); ?> <?php walker_edge_inline_style( $social_icon_styles ) ?> href="<?php echo esc_html( $link ); ?>" target="<?php echo esc_attr( $target ); ?>">
			<?php echo walker_edge_get_module_part( $icon_holder_html ); ?>
		</a>
		<?php
	}
}