...
if (!function_exists('walker_edge_woocommerce_loop_add_to_cart_link')) {
	...
	function walker_edge_woocommerce_loop_add_to_cart_link() {
		...
		$class = implode(' ', array_filter( array('button',	'product_type_' . $product->get_type(), $product->is_purchasable() && $product->is_in_stock() ? 'add_to_cart_button' : '', $product->supports( 'ajax_add_to_cart' ) ? 'ajax_add_to_cart' : '' ) ) );
