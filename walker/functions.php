...
if ( ! function_exists( 'walker_edge_theme_setup' ) ) {
	...
	function walker_edge_theme_setup() {
		...
		$GLOBALS['content_width'] = apply_filters( 'walker_edge_filter_set_content_width', 1100 );
...
if ( ! function_exists( 'walker_edge_styles' ) ) {
	...
	function walker_edge_styles() {
		...
		wp_enqueue_style( 'walker-default-style', EDGE_ROOT . '/style.css' );
		wp_enqueue_style( 'walker-edge-modules', EDGE_ASSETS_ROOT . '/css/modules.min.css' );

		//is woocommerce installed?
		if ( walker_edge_is_woocommerce_installed() && walker_edge_load_woo_assets() ) {
			//include theme's woocommerce styles
			wp_enqueue_style( 'walker-edge-woo', EDGE_ASSETS_ROOT . '/css/woocommerce.min.css' );
		}
		...
		if ( walker_edge_is_responsive_on() ) {
			wp_enqueue_style( 'walker-edge-modules-responsive', EDGE_ASSETS_ROOT . '/css/modules-responsive.min.css' );

			//is woocommerce installed?
			if ( walker_edge_is_woocommerce_installed() && walker_edge_load_woo_assets() ) {
				wp_enqueue_style( 'walker-edge-woo-responsive', EDGE_ASSETS_ROOT . '/css/woocommerce-responsive.min.css' );
				$style_dynamic_deps_array = array( 'walker-edge-woo', 'walker-edge-woo-responsive' );
			}

			if ( file_exists( EDGE_ROOT_DIR . '/assets/css/style_dynamic_responsive.css' ) && walker_edge_is_css_folder_writable() && ! is_multisite() ) {
				wp_enqueue_style( 'walker-edge-style-dynamic-responsive', EDGE_ASSETS_ROOT . '/css/style_dynamic_responsive.css', array(), filemtime( EDGE_ROOT_DIR . '/assets/css/style_dynamic_responsive.css' ) );
			} else if ( file_exists( EDGE_ROOT_DIR . '/assets/css/style_dynamic_responsive_ms_id_' . walker_edge_get_multisite_blog_id() . '.css' ) && walker_edge_is_css_folder_writable() && is_multisite() ) {
				wp_enqueue_style( 'walker-edge-style-dynamic-responsive', EDGE_ASSETS_ROOT . '/css/style_dynamic_responsive_ms_id_' . walker_edge_get_multisite_blog_id() . '.css', array(), filemtime( EDGE_ROOT_DIR . '/assets/css/style_dynamic_responsive_ms_id_' . walker_edge_get_multisite_blog_id() . '.css' ) );
			} else {
				wp_enqueue_style( 'walker-edge-style-dynamic-responsive', EDGE_ASSETS_ROOT . '/css/style_dynamic_responsive.php' );
			}
		}
		...
		if ( file_exists( EDGE_ROOT_DIR . '/assets/css/style_dynamic.css' ) && walker_edge_is_css_folder_writable() && ! is_multisite() ) {
			wp_enqueue_style( 'walker-edge-style-dynamic', EDGE_ASSETS_ROOT . '/css/style_dynamic.css', array(), filemtime( EDGE_ROOT_DIR . '/assets/css/style_dynamic.css' ) ); //it must be included after woocommerce styles so it can override it
		} else if ( file_exists( EDGE_ROOT_DIR . '/assets/css/style_dynamic_ms_id_' . walker_edge_get_multisite_blog_id() . '.css' ) && walker_edge_is_css_folder_writable() && is_multisite() ) {
			wp_enqueue_style( 'walker-edge-style-dynamic', EDGE_ASSETS_ROOT . '/css/style_dynamic_ms_id_' . walker_edge_get_multisite_blog_id() . '.css', array(), filemtime( EDGE_ROOT_DIR . '/assets/css/style_dynamic_ms_id_' . walker_edge_get_multisite_blog_id() . '.css' ) );
		} else {
			wp_enqueue_style( 'walker-edge-style-dynamic', EDGE_ASSETS_ROOT . '/css/style_dynamic.php' ); //it must be included after woocommerce styles so it can override it
		}
	}
...
if ( ! function_exists( 'walker_edge_google_fonts_styles' ) ) {
	...
	function walker_edge_google_fonts_styles() {
		...
		if ( count( $fonts_array ) > 0 ) {
			...
			wp_enqueue_style( 'walker-edge-google-fonts', esc_url_raw( $walker_edge_fonts ), array(), '1.0.0' );
...
if ( ! function_exists( 'walker_edge_scripts' ) ) {
	...
	function walker_edge_scripts() {
		...
		wp_enqueue_script( 'touchswipe', EDGE_ASSETS_ROOT . '/js/plugins/jquery.touchSwipe.min.js', array( 'jquery' ), false, true );
		...
		if ( walker_edge_options()->getOptionValue( 'google_maps_api_key' ) != '' ) {
			...
			wp_enqueue_script( 'walker-google-map-api', 'https://maps.googleapis.com/maps/api/js?key=' . $google_maps_api_key, array(), false, true );
		}

		wp_enqueue_script( 'walker-edge-modules', EDGE_ASSETS_ROOT . '/js/modules.min.js', array( 'jquery' ), false, true );
...
if ( ! function_exists( 'walker_edge_get_page_id' ) ) {
	...
	function walker_edge_get_page_id() {
		if ( walker_edge_is_woocommerce_installed() && ( walker_edge_is_woocommerce_shop() || walker_edge_is_product_category() || walker_edge_is_product_tag() ) ) {
...
if ( ! function_exists( 'walker_edge_sidebar_layout' ) ) {
	...
	function walker_edge_sidebar_layout() {
		...
		if ( ( $page_sidebar_meta !== '' ) && $page_id !== - 1 ) {
			...
			} elseif ( is_archive() && walker_edge_is_woocommerce_page() ) {
				if ( is_product_category() || is_product_tag() ) {
					$shop_id        = get_option( 'woocommerce_shop_page_id' );
					$sidebar_layout = walker_edge_get_meta_field_intersect( 'sidebar', $shop_id );
				}
		...
		if ( ! empty( $sidebar_layout ) && ! is_active_sidebar( walker_edge_get_sidebar() ) ) {
			$sidebar_layout = '';
		}
...
if ( ! function_exists( 'walker_edge_page_custom_style' ) ) {
	...
	function walker_edge_page_custom_style() {
		...
		$style = apply_filters( 'walker_edge_add_page_custom_style', $style = array() );

		if ( $style !== '' ) {
			wp_add_inline_style( 'walker-edge-modules', $style );
		}
	}
...
if ( ! function_exists( 'walker_edge_get_unique_page_class' ) ) {
	...
	function walker_edge_get_unique_page_class() {
		...
		if ( walker_edge_is_woocommerce_installed() && is_product() ) {
			$id = get_the_ID();
		}
		...
		if ( is_single() ) {
			...
			$page_class .= '.home';
		} elseif ( is_archive() || $id === walker_edge_get_woo_shop_page_id() ) {
			$page_class .= '.archive';
		} elseif ( is_search() ) {
			$page_class .= '.search';
		} elseif ( is_404() ) {
...
if ( ! function_exists( 'walker_edge_print_custom_css' ) ) {
	...
	function walker_edge_print_custom_css() {
		...
		if ( $custom_css !== '' ) {
			wp_add_inline_style( 'walker-edge-modules', $custom_css );
		}
	}
...
if ( ! function_exists( 'walker_edge_print_custom_js' ) ) {
	...
	function walker_edge_print_custom_js() {
		...
		if ( $custom_js !== '' ) {
			wp_add_inline_script( 'walker-edge-modules', $custom_js );
		}
	}
...
if ( ! function_exists( 'walker_edge_get_global_variables' ) ) {
	...
	function walker_edge_get_global_variables() {
		...
		wp_localize_script( 'walker-edge-modules', 'edgtfGlobalVars', array(
...
if ( ! function_exists( 'walker_edge_per_page_js_variables' ) ) {
	...
	function walker_edge_per_page_js_variables() {
		...
		wp_localize_script( 'walker-edge-modules', 'edgtfPerPageVars', array(
...
if ( ! function_exists( 'walker_edge_get_module_part' ) ) {
	function walker_edge_get_module_part( $module ) {
		return $module;
	}
}
...
if ( ! function_exists( 'walker_edge_is_gutenberg_installed' ) ) {
    /**
     * Function that checks if Gutenberg plugin installed
     * @return bool
     */
    function walker_edge_is_gutenberg_installed() {
        return function_exists( 'is_gutenberg_page' ) && is_gutenberg_page();
    }
}

if ( ! function_exists( 'walker_edge_is_wp_gutenberg_installed' ) ) {
    /**
     * Function that checks if WordPress 5.x with Gutenberg editor installed
     * @return bool
     */
    function walker_edge_is_wp_gutenberg_installed() {
