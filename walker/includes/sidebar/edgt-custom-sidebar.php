...
if( ! class_exists('WalkerEdgeClassSidebar') ){
	class WalkerEdgeClassSidebar{
		var $sidebars  = array();
		var $stored    = "";
		...
		function __construct(){
			...
			$this->title = esc_html__('Custom Widget Area','walker');
		...
		function load_assets(){
			...
			wp_enqueue_script('walker-admin-sidebar' , EDGE_ROOT . '/framework/admin/assets/js/edgtf-sidebar.js');
			wp_enqueue_style( 'walker-admin-sidebar' , EDGE_ROOT . '/framework/admin/assets/css/edgtf-sidebar.css');
		}
		...
		function template_add_widget_field(){
			...
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '".esc_html__('Enter Name of the new Widget Area', 'walker')."' name='edgt-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='".esc_html__('Add Widget Area', 'walker')."' />";
			echo "\n    ".$nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
		...
		function delete_sidebar_area(){
			...
			if(!empty($_POST['name'])){
				...
				if(($key = array_search($name, $this->sidebars)) !== false){
					...
					update_option($this->stored, $this->sidebars);
					echo esc_html__( "sidebar-deleted", 'walker' );
				}
			}
		...
		function register_custom_sidebars(){
			...
			$args = apply_filters('walker_edge_custom_widget_args', $args);
